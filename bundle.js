  ...
  _createClass(OAuth2Manager, [{
    ...
    value: function refreshOAuth2Token() {
      ...
      if (!isNaN(that.oauth2Token.expires_in) && typeof oauth2Config.refreshToken === "function") {
        var endsIn = that.oauth2Token.expires_at !== undefined ? Math.max(0, Math.floor((new Date(that.oauth2Token.expires_at).getTime() - new Date().getTime()) / 1000)) : Math.max(that.oauth2Token.expires_in, that.oauth2Token.expires_in - 100);
    ...
    */
    ...
    value: function callApi(path, httpMethod, pathParams, queryParams, headerParams, formParams, bodyParam, authNames, contentTypes, accepts, returnType) {
      ...
      if (typeof headerParams['User-Agent'] === 'undefined') {
        headerParams['User-Agent'] = 'autodeskforgedesignautomation-js-sdk' + '/3.0.2' + ' (Forge RSDK)';
      } else {
        var userAgent = headerParams['User-Agent'];
        userAgent = userAgent.concat(' autodeskforgedesignautomation-js-sdk').concat('/3.0.2').concat(' (Forge RSDK)');
